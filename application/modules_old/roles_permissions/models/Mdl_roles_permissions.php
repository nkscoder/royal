...
class Mdl_roles_permissions extends CI_Model{
    ...
    public function addPermissionsToRoles(){
        $this->_validate();
        $data=[
            'eduworkers_roles_permissions_role_id'=>$this->roles_id,
            'eduworkers_roles_permissions_permission_id'=>$this->permissions_id
        ];
        if($this->db->insert('eduworkers_roles_permissions',$data)){
            return true;
        }
        return false;
    }
